...
/-! ## THE ORIENTATION DEFECT IN THE FIRST VERSION OF THIS HANDOFF, AND THE REPAIR.

`o06_angle_sum_of_polar_fan` above demanded ASCENDING central arguments together with
`InsideAngleAt`. Ascending arguments is the COUNTERCLOCKWISE listing; `InsideAngleAt`
requires the turn at each vertex to be non-negative, which selects the CLOCKWISE one. On a
convex hull listing the two are mutually unsatisfiable -- the theorem is true and its
hypotheses cannot both hold. Measured on the unit square about the origin: counterclockwise
gives `InsideAngleAt` FALSE at all four vertices, clockwise TRUE at all four. The socket was
offered to the producing lane UNMODELLED, which is why it survived; a model shows it in one
line, as it did the moment one was attempted.

`hcentral_of_polar` survives the repair untouched, because the central angle is
`arccos (cos d)` and cosine is EVEN -- only the ceiling `|d| <= pi` matters, never the
direction. And this version IS instantiated, at the bottom of this file.
-/
...
/-- THE ORIENTATION-CORRECT HANDOFF: descending central arguments. -/
...
/-! ## THE MODEL FOR THE REPAIRED HANDOFF: the clockwise square with its polar data. -/
...
/-- THE REPAIRED HANDOFF, INSTANTIATED. All four hypothesis families hold TOGETHER on the
clockwise square, which is exactly what the first version could not do. -/
theorem o06_polar_fan_desc_model :
    ∑ i ∈ Finset.range 4,
        EuclideanGeometry.angle (ww i) (ww (i + 1)) (ww (i + 2)) = ((4 : ℝ) - 2) * Real.pi := by
  have hpi := Real.pi_pos
  have hgap : ∑ i ∈ Finset.range 4, (aa i - aa (i + 1)) = 2 * Real.pi := by
    ...
    ring
  simpa using
    o06_angle_sum_of_polar_fan_desc 4 (by norm_num) (pt2 0 0) ww aa (fun _ => 1)
      ww_per (fun _ => by norm_num) ww_polar0 ww_polar1
      (fun i _ => by rw [aa_gap i]; linarith)
      (fun i _ => by rw [aa_gap i]; linarith)
      hgap ww_inside

#print axioms hcentral_of_polar_desc
#print axioms o06_angle_sum_of_polar_fan_desc
#print axioms o06_polar_fan_desc_model
