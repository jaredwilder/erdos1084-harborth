...
/-! ## THE MODEL -- the frontier hypotheses are NOT vacuous.

Campaign law: a socket offered UNMODELLED is how a false statement survives.  The clockwise
unit square about its centre, taken as points of the closed unit BALL: `‖x‖ = 1` puts a point
out of the open ball, which is exactly the ball's interior.  No extremality is used anywhere. -/
...
theorem ww_pol0 (i : ℕ) : (ww i).ofLp 0 = (0 : ℝ^ 2).ofLp 0 + 1 * Real.cos (aa i) := by
...
theorem ww_pol1 (i : ℕ) : (ww i).ofLp 1 = (0 : ℝ^ 2).ofLp 1 + 1 * Real.sin (aa i) := by
  have h : i % 4 = 0 ∨ i % 4 = 1 ∨ i % 4 = 2 ∨ i % 4 = 3 := by omega
  rw [aa_sin i, zero1]
  rcases h with h | h | h | h <;>
...
theorem ww_norm (i : ℕ) : ‖ww i‖ = 1 := by
  ...
  rw [zero1] at h1
  rw [h0, h1]
  rw [show (0 + 1 * Real.cos (aa i)) ^ 2 + (0 + 1 * Real.sin (aa i)) ^ 2 = 1 by
    have := Real.sin_sq_add_cos_sq (aa i); nlinarith [this]]
  exact Real.sqrt_one

theorem ww_mem (i : ℕ) : ww i ∈ Metric.closedBall (0 : ℝ^ 2) 1 := by
  simp [Metric.mem_closedBall, dist_zero_right, ww_norm i]

theorem ww_notint (i : ℕ) : ww i ∉ interior (Metric.closedBall (0 : ℝ^ 2) 1) := by
  rw [interior_closedBall _ (by norm_num : (1:ℝ) ≠ 0)]
  simp [Metric.mem_ball, dist_zero_right, ww_norm i]

/-- ⛔ THE MODEL, END TO END, ON THE FRONTIER STATEMENT. -/
theorem e1084_frontier_angle_sum_square :
    ∑ i ∈ Finset.range 4,
  ...
  have hgap : ∑ i ∈ Finset.range 4, (aa i - aa (i + 1)) = 2 * Real.pi := by
    ...
    ring
  have hint : (0 : ℝ^ 2) ∈ interior (Metric.closedBall (0 : ℝ^ 2) 1) := by
    rw [interior_closedBall _ (by norm_num : (1:ℝ) ≠ 0)]
    simp [Metric.mem_ball]
  simpa using
    e1084_frontier_angle_sum (Metric.closedBall (0 : ℝ^ 2) 1) (convex_closedBall _ _)
      4 (by norm_num) 0 hint ww aa (fun _ => 1)
      ww_mem ww_notint ww_per (fun _ => by norm_num) ww_pol0 ww_pol1
      (fun i => by rw [aa_gap i]; linarith)
      (fun i => by rw [aa_gap i]; linarith)
      hgap

#print axioms extremePoints_not_interior
#print axioms extremePoint_mem_frontier
#print axioms vertexPts_subset_hullPts
#print axioms convexHull_subset_hullPts
#print axioms s2_sides_to_polar_ge
#print axioms s1_frontier_signs
#print axioms h3_frontier_inside
#print axioms e1084_frontier_angle_sum
#print axioms e1084_hullPts_angle_sum
#print axioms e1084_frontier_angle_sum_square
